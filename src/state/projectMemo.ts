 ...
 * 없음).
 *
 * **알려진 한계 — 창을 넘는 중복**: "프로젝트당 1개"의 조회 범위는 이 웹뷰다
 * (`surfaceRegistry`는 창별 싱글턴이라 주·부 두 dock은 덮지만 팝아웃 창은 못
 * 본다 — 타임라인 peek·attention 롤업과 같은 계열의 한계). 메모 탭을 다른 창으로
 * 끌어낸 뒤 메인 창에서 다시 열면 같은 memo.md를 보는 에디터가 둘 생기고, 둘 다
 * 화면에 떠 있는 동안의 편집은 나중에 저장한 쪽이 이긴다. 한 창 안에서는 발생하지
 * 않는다(비활성 탭은 언마운트되고 재활성화 때 디스크에서 다시 읽는다). 창 간 이동
 * 자체를 막을지는 미결 — peek처럼 단발성으로 분류하면 레이아웃 복원에서도 함께
 * 닫혀 버리므로(두 규칙이 `ephemeralKindOf` 하나를 공유한다) 그 길은 못 쓴다.
 */
