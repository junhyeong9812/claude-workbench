...
/// 리터럴이 있는지 먼저 확인한다.
///
/// 남는 구멍과 그 판단: 본문이 "위에 보인 id를 그대로 조립해 종료 태그를 만들라"고
/// 모델에게 시키면 nonce도 우회될 수 있다. **수용한다** — 그 문자열을 메모에 적는
/// 사람은 사용자 자신이고(이 문서는 남이 보내온 입력이 아니라 본인의 메모다),
/// 성공해도 얻는 것은 자기 메모의 정리 결과가 이상해지는 것뿐이다. 실제로 막아야
/// 하는 것은 악의가 아니라 **사고성 절단**이고, 그건 nonce(우연 충돌 제거)와
/// 절단 경고 배지·`truncated` 오류가 덮는다. 여기서 더 나가려면 본문을 파일로
/// 넘기는 등 표면을 바꿔야 하는데, 그건 이 기능이 지불할 비용이 아니다.
